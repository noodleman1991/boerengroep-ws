...
export function CalendarProvider({
    ...
    const [settings, setSettings] = useLocalStorage<CalendarSettings>(
        "calendar-settings",
        {
            ...DEFAULT_SETTINGS,
            badgeVariant: badge,
            view: view === "day" || view === "week" || view === "year" ? "month" : view, // Force supported views
        },
    );
    ...
    const [currentView, setCurrentViewState] = useState<TCalendarView>(
        settings.view === "day" || settings.view === "week" || settings.view === "year"
            ? "month"
            : settings.view
    );
    ...
    const setView = (newView: TCalendarView) => {
        // Only allow month and agenda views
        if (newView !== "month" && newView !== "agenda") {
            console.warn(`View "${newView}" is not supported. Defaulting to month view.`);
            newView = "month";
        }
